...
for y in range(2006, 2016):
  ...
  for d in divisions:
    ...
    div = json.loads(open('divisions/senate-div-' + str(d['id']) + '.json').read())
    div_detail[d['id']] = div
  ...
  # Save the graph
  nx.write_gexf(graph, 'senate-graph-' + str(y) + '.gexf')
  with open('../../static/senate-'+str(y)+'.json', 'w') as of:
    json.dump(js, of)
